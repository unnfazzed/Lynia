...
| 5 | Cancel (both roles) | Copy, enum leak | `apps/api/src/orders/order-lifecycle.service.ts:454` | `ConflictException` message included the raw status enum verbatim (e.g. "Cannot cancel a en_route_dropoff order") — the client renders exception messages unmodified. | Plain-language message: "This order can't be cancelled anymore — it's already past that point." | Med-High | S |
