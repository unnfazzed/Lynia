...
**Last updated:** 2026-07-19 (third run executed — see `docs/REFACTOR-2026-07-19.md`).
...
| 5 | `apps/mobile/src/auth/session.ts` | 4, steady 3-run riser | 585 lines, mixes the real auth session with ~10 unrelated per-order/per-flow SecureStore groups | OPEN — RF-10, scoped this run (see debt register), queued for next run — only 4 existing tests, needs 1 characterization test first |
...
| 7 | `apps/api/src/tracking/tracking.gateway.ts` | grew to 770 lines (was 708) | mixes socket handlers/heartbeat/emit-coalescing over shared mutable state | OPEN — RF-05, still needs a design pass (too large for one PR) |
...
| RF-10 | `apps/mobile/src/auth/session.ts` | oversized | 585 lines, ~35 exports mixing the real auth `Session` (load/save/clear/deviceId) with ~10 unrelated per-order/per-flow SecureStore groups (delivery-code trio, confirmItemsPending, pendingRating, riderJobTerminal, senderRatingPending, pendingTopup, rolePreference, onboarding/permissions/disclaimer flags, handback acks) plus `clearDeviceState()`. Scoped plan: move the non-auth groups to a new `device-state.ts`, re-export via `export * from "./device-state"` from `session.ts` so none of the 15 importers (including sensitive `order/[id].tsx`/`rider/job.tsx`) change. `session.test.ts` has only 4 tests — needs one characterization test first ("`clearDeviceState()` wipes every persisted key this module defines", which also structurally closes the BH-15/17/19 recurring "sign-out wipe missed a key" class). | M | OPEN — scoped 2026-07-19, ready for next run (not done this run: harness constrained this session to a single PR) | — |
