...
| 6 | `apps/api/src/privacy/privacy.service.ts` | 6 (+6 spec) | 441 lines; `eraseAccount` alone was a 300-line method (preflight → CAS anonymise → rider scrub → PII sweep → GCS purge) | **RF-11 DONE this run** (2026-07-21) — see completed log. Split into `preflight`/`anonymiseProfileTx`/`scrubPiiTx`/`postCommitPurge`, all CAS/TOCTOU comments carried verbatim, no reordering; still sensitive-adjacent, unchanged behavior |
