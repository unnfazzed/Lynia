...
| LR11 | Perf | Load-test harness (k6) + staging stack; SLO table measured green | agent | 🛠️ **k6 harness authored** (`apps/api/load/`); staging stack applied and armed (`docs/GCP-PENDING-REVIEW-2026-07-13.md` §Appendix) — needs OTEL live (LR9, founder) to run and measure |
...
| LR20 | UI | Crash telemetry + store readiness (listing, privacy, data-safety) | agent + founder | 🛠️ **Store readiness: DONE (2026-08-04)** — the app is live on Play's **internal testing** track, v0.17.9 / versionCode 2, submitted end-to-end by the pipeline (build `c248fbf5` + submission `574bf5fd`; `docs/PLAY-STORE-SUBMISSION.md` attempt-9 block). Listing copy, data-safety answers, declarations, privacy/deletion pages, in-app account deletion, graphics all shipped. **Crash telemetry: wiring DONE (2026-08-05), activation pending** — the dedicated RN project `lyniago/lynia-mobile` exists and is committed as `app.config.ts` plugin props with `experimental_android.enableAndroidGradlePlugin` (R8 `mapping.txt` + native symbol upload, without which `enableProguardInReleaseBuilds` leaves every Java/Kotlin frame obfuscated); `SENTRY_DISABLE_AUTO_UPLOAD` is removed from `eas.json`; both release profiles pin an explicit EAS `environment`; `mobile-ota.yml` + `android-test-apk.yml` pass the DSN through (they export on the runner, so an OTA would otherwise have silently switched crash reporting OFF); and a release build now hard-fails without `EXPO_PUBLIC_SENTRY_DSN`/`SENTRY_AUTH_TOKEN` instead of shipping blind. **Not yet met:** the founder's EAS variables + GitHub secret, and the device exit test (long-press the TEST BUILD banner → fire BOTH the JS error and the native crash). The live binary still reports nothing — telemetry reaches devices only in a new build. §8 step 3 (production staged rollout) still requires that test green |
