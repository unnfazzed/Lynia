...
| [`SECURITY.md`](./SECURITY.md) | 🟢 Living (spec) | Security plan & threat model: data classification, threat actors/trust boundaries, a STRIDE attack-surface map, an honest posture scorecard, and a prioritized P0→P3 remediation roadmap (each item grounded in real `file:line` with an acceptance test). Defense-in-depth, data-protection/privacy, and the incident-response + governance lifecycle. Companion to the root [`../SECURITY.md`](../SECURITY.md) disclosure policy. |
