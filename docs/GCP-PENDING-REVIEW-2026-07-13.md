...
| **Mobile/EAS pipeline** | No Play builds / OTA from CI | **RESOLVED for the build/submit half, 2026-08-04** — `EAS_RELEASE_ENABLED=true`, and the pipeline shipped v0.17.9 (vc 2) to Play's internal track end-to-end (`docs/PLAY-STORE-SUBMISSION.md`). GCP-side items this needed, both now done: **`androidpublisher.googleapis.com` enabled** in project `407250490173` (it had never been enabled, which is what `PERMISSION_DENIED` on the first auto-submit actually meant), and the Play publisher SA `id-play-publisher@lynia-500911.iam.gserviceaccount.com` — whose key required a **temporary project-scoped lift of the org `iam.disableServiceAccountKeyCreation` policy**, re-enforced immediately after (2026-08-03). ⚠️ That SA key is now a long-lived credential in EAS: it belongs on the rotation schedule (`docs/SECRET-ROTATION.md`) and holds testing-track permissions only — production-release permission is still deliberately withheld. **OTA from CI is still not working** (`REL-01`/`REL-02`, `docs/KNOWN_BUGS.md`) — not a GCP-side gap |
