...
| DOC-16-01 | `top_ups.phone` (the mobile-money number captured on every self-serve wallet top-up, `wallet.service.ts:300`) is never scrubbed on account erasure or by the retention sweep — `PrivacyService.eraseAccount`/`purgeExpiredData` (`privacy.service.ts`) have zero reference to `topUp`/`commissionAccount`/`commissionLedger`, unlike every other dialable-phone field in the system (`profiles.phone`, the order-embedded `contactPhone`), which `docs/DATA-RETENTION.md` Principle 2 commits to scrubbing on erasure | `apps/api/src/privacy/privacy.service.ts`, `apps/api/src/wallet/wallet.service.ts:300`, `apps/api/prisma/schema.prisma:329` (`TopUp.phone`) | LOW-MED (CDPA erasure-completeness gap; narrow surface, not independently exploitable) | 1 | Found by the 2026-07-16 doc-sync routine while reconciling `docs/DATA-RETENTION.md`'s new wallet-table row against `PrivacyService`. Owning lane: WD (wallet & data-lifecycle audit) — fix is to null `top_ups.phone` in `eraseAccount` (mirroring the existing `contactPhone` read-modify-write) and, if desired, in `purgeExpiredData` for terminal (confirmed/expired/declined) intents. |
| DOC-16-02 | **Self-serve wallet top-up is non-functional on all three rails — every attempt guarantees a 90s timeout, and EcoCash never got the direct integration the design explicitly mandated.** `WalletService.creditFromTopup` (`wallet.service.ts:409`) is the only code path that ever transitions a `TopUp` out of `pending`/`expired` into `confirmed` and writes the credit — a repo-wide grep confirms it has **zero callers** anywhere outside its own file. `apps/mobile/app/wallet/top-up.tsx` polls `GET /wallet/topups/:id` for up to 90s (`TOPUP_WINDOW_MS`) waiting for `status:"succeeded"`, which can never arrive, so every top-up attempt on every rail silently times out (`step="timeout"`) with no way to ever succeed. This isn't just an unfinished stub: `docs/plans/2026-rider-wallet-design.md` Premise 4 (agreed 2026-07-13, "founder call D5 — upheld against the cross-model challenge") explicitly commits to **"Direct EcoCash integration in this build"**, with InnBucks/O'mari served *only* by the manual admin-credit path (never a fake self-serve form) — but `grep -rli ecocash apps/api/src` finds no payment-gateway client anywhere, just a UI label string (`wallet.service.ts:36`), and `packages/shared/src/contracts.ts:452`'s `SELF_SERVE_RAILS = ["ecocash","innbucks","omari"]` + `top-up.tsx`'s identical amount+phone+90s-wait UI treats all three rails the same, contradicting both Premise 4 and the design's own UI Spec (`design.md` "Top-up (manual)" row: InnBucks/O'mari should render an **instruction card**, never a payment form). Because `WALLET_REVEAL` now defaults to `"true"` (`env.ts:189`, product decision 2026-07-15), this broken flow is live and reachable by real riders today, even at 0% commission. | `apps/api/src/wallet/wallet.service.ts:409`, `apps/mobile/app/wallet/top-up.tsx`, `packages/shared/src/contracts.ts:452`, `docs/plans/2026-rider-wallet-design.md` Premise 4 (line 83-86) + UI Spec (Top-up rows) | HIGH (live, user-facing, permanently-broken money flow — not a backend integrity issue since no money ever moves, but a trust/functionality break exactly the design brief's own north star warns against) | 1 | Found by the 2026-07-16 doc-sync routine while reconciling `docs/plans/2026-rider-wallet-design.md` against the shipped wallet code. Owning lane: WD (wallet & data-lifecycle audit) — fix is either (a) wire a real EcoCash C2B client + `creditFromTopup` caller per Premise 4, or (b) at minimum, ship InnBucks/O'mari (and EcoCash, until the integration lands) as the design's instruction-card flow routing riders to the working admin-manual-credit path instead of a fake self-serve form that can never succeed. |
| DOC-16-06 | `docs/plans/2026-rider-wallet-design.md` Premise 6 ("The dormant `Settlement` table is dropped in this build's migration") was not executed — `schema.prisma:356` still defines `model Settlement` (comment: "DORMANT... will be dropped... when the wallet is built"), and `Refund.settlementId` + its index are still present (`schema.prisma:613,617`). Migration `0027_commission_wallet/migration.sql:3-4` explains the deferral: "left in place (its drop is a separate destructive migration once the wallet has soaked)." Zero functional impact — the table is unread/unwritten either way — but it's an un-executed agreed premise, not yet re-confirmed as an intentional change. | `apps/api/prisma/schema.prisma:356-378,613,617`, `apps/api/prisma/migrations/0027_commission_wallet/migration.sql:1-4`, `docs/plans/2026-rider-wallet-design.md` Premise 6 (line 92) | LOW (housekeeping only, no behavior/data risk) | 1 | Found by the 2026-07-16 doc-sync routine. Owning lane: WD (wallet & data-lifecycle audit) — either drop `Settlement`/`Refund.settlementId` in a dedicated destructive migration now that the wallet has soaked through PR1 + WD-001..011 + BH-07..12, or explicitly re-affirm the deferral as current policy and update Premise 6's wording (doc-sync won't do this itself — editing an agreed premise is out of scope for a doc-only pass). |
| DOC-16-05 | KYC duplicate-ID dedup (`rider.service.ts:51-56` `duplicateIdAccountCount`, A-04) is advisory-only: `completeProfile`/`becomeRider` stamp `duplicateIdFlag` and surface it to a human via `admin-riders.service.ts:101` `getKycReview`, but `applyKycResult` (the vendor auto-mode webhook path, `rider.service.ts:386-`) never reads the flag — under the default `KYC_MODE=auto`, a banned/suspended rider re-registering with a new SIM and the same real ID/face sails straight to `verified` with no human ever seeing the flag. Distinct from DS-11 (which freezes `idNumber` on an *existing* verified account) — this is about a *fresh* account re-using a banned identity. | `apps/api/src/riders/rider.service.ts:51-56,141-143,386-`, `apps/api/src/admin/admin-riders.service.ts:101` | MEDIUM (ban-evasion path, requires the evader to still hold their real ID/face — Didit verification isn't spoofable, so this is a re-registration loophole, not an identity-forgery one) | 1 | Found by the 2026-07-16 doc-sync routine while re-verifying `docs/FRAUD-REVIEW.md` P1-3 against current `rider.service.ts`. Owning lane: DS (deep sweep — KYC-gate bypass). Fix: have `applyKycResult`'s auto-verify branch consult `duplicateIdFlag` and route a match to manual review instead of auto-approving. |
| DOC-16-04 | **The commission basis has no floor — the pre-wallet fare-lowballing exploit (`docs/FRAUD-REVIEW.md` P0-2) is confirmed still open in the shipped wallet code, dormant only because `COMMISSION_RATE_PCT` defaults to 0%.** `WalletService.chargeCommission` (`wallet.service.ts:186-245`) computes `perRideCommission(agreedFare, rate)` straight off `Order.agreedFare`, which is copied verbatim from the rider's `offeredFare` (`matching.service.ts:129`) whose only bound is `positive, ≤ 100_000, 2dp` (`contracts.ts:91`) — no floor tying it to `suggestedFare`/distance. A colluding customer+rider pair can agree the real fare off-app, bid `offeredFare=$0.01`, and owe near-zero commission once the rate is live. The same no-floor gap exists on the correction path: `AdminOrdersService.adjustFare` (`admin-orders.service.ts:179-225`) accepts any positive `agreedFare` with no downward bound, and `WalletService.adjustCommissionInTx` (`wallet.service.ts:257-280`) writes whatever signed `adjustment` delta results, uncapped. | `apps/api/src/wallet/wallet.service.ts:186-245,257-280`, `apps/api/src/matching/matching.service.ts:129`, `packages/shared/src/contracts.ts:91`, `apps/api/src/admin/admin-orders.service.ts:179-225` | HIGH, but **dormant pre-flip** — zero financial impact while `COMMISSION_RATE_PCT=0`; must close before the rate is ever set > 0 | 1 | Found by the 2026-07-16 doc-sync routine while re-checking `docs/FRAUD-REVIEW.md` P0-2/P2-1's own "worth re-checking once the wallet ships" note against the now-shipped `wallet.service.ts`. Owning lane: WD (wallet & data-lifecycle audit) — fix per FRAUD-REVIEW.md P0-2's suggested fix: bill commission on `max(agreedFare, floor(suggestedFare))` and/or reject `offeredFare` below a policy band of `suggestedFare`, applied consistently in both `chargeCommission` and `adjustCommissionInTx`. Must land before any flip-day rehearsal sets `COMMISSION_RATE_PCT` > 0 anywhere the flip could reach production. |
| DOC-16-03 | Admin console has no UI for the wallet at all, despite the backend routes existing — `POST /admin/riders/:id/wallet-credit` (`admin.controller.ts:275`, calls `wallet.service.ts:443` `creditManual`) is reachable only via raw API call. `apps/admin/app/riders/[id]/page.tsx:136,181-183` shows only the read-only `commission` accrual projection (from `settlements.service.ts`'s `commissionOverview`, not the wallet balance) with no ledger table and no credit form; a repo-wide grep for `wallet-credit`/`creditWallet`/`wallet_credit` across `apps/admin` returns nothing. `docs/plans/2026-rider-wallet-design-brief.md` §"Admin console — rider detail" (lines 95-100) specs wallet balance + ledger table + a manual-credit form (capped $50, form-open idempotency key, frozen-state rendering) as part of this build; §"Admin console — Commission page" (lines 101-104) specs the **bulk seed-credit** action (campaign name, per-rider amount, preview count, re-run dedup message) as in-scope for the flip-day tooling `design.md` Premise 5 promises ("bulk seed-credit action... are in this build"). Neither exists anywhere in `apps/api/src` or `apps/admin` (grep for `seed-credit`/`campaignId` is empty). | `apps/admin/app/riders/[id]/page.tsx`, `apps/api/src/admin/admin.controller.ts:275`, `docs/plans/2026-rider-wallet-design-brief.md` lines 95-104, `docs/plans/2026-rider-wallet-design.md` Premise 5 | MEDIUM (ops can't actually run the launch top-up rail or the flip-day rehearsal without hand-rolling API calls; not user-facing, no data-integrity risk) | 1 | Found by the 2026-07-16 doc-sync routine while reconciling the wallet design brief against `apps/admin`. Owning lane: WD (wallet & data-lifecycle audit). |
