...
| KB-CI-AUDIT-410 | `.github/workflows/ci.yml`'s `security` job hard-fails on **every** PR/push: npmjs.org retired both `pnpm audit` endpoints (quick + bulk both return HTTP 410, `ERR_PNPM_AUDIT_BAD_RESPONSE`), unrelated to any real advisory. Confirmed on [PR #249](https://github.com/unnfazzed/Lynia/pull/249)'s `dependency audit · secret scan` check (2026-07-15). This blocks auto-merge repo-wide, including this ledger update's own PR, until resolved. | `.github/workflows/ci.yml` | HIGH (blocks all shipping) | 1 | **Founder-gated, not auto-fixed this run:** the PR-health watchdog drafted a fix (treat the specific 410/`ERR_PNPM_AUDIT_BAD_RESPONSE` response as a non-blocking warning, still hard-fail on any other `pnpm audit` failure — i.e. a real advisory) but the run's own safety classifier withheld the edit because it weakens a security gate with no human present to confirm. See the 2026-07-15 PR-health report for the exact proposed diff; needs a human to review/apply/land it, or to pick an alternative SCA tool not dependent on the retired npm endpoint. **Independently re-confirmed by the 2026-07-15 doc-sync routine** (also observed on `main` itself, runs `29390793070`/`29390762282`) — same root cause, no new information. |
