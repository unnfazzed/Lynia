...
| LC-D05 | Busy-mode / back-in-stock / starter-category taps `try/finally` with no `catch` — do nothing, say nothing on a network drop. | `apps/merchant/app/(app)/hours/page.tsx:408`, `menu/page.tsx` | MEDIUM | **FIXED** (LC loop D, 2026-08-02) — busy-mode (`HoursPage.onToggleBusy`) and back-in-stock (`MenuPage.onClearOos`) were closed by LC-D04. The remaining scope, `MenuPage.onCreateStarterCategory`'s bare `try { … } catch { /* best-effort */ }`, now surfaces the failure via the same `listError` state `onClearOos` uses (rendered in the empty-state card the starter chips live in), instead of silently leaving the chip tappable again with zero signal. Regression tests in `menu/page.test.tsx` (confirmed they fail against the pre-fix code). See `docs/LC-D-REPORT-2026-08-02b.md`. |
