...
**Deliberately NOT done (follow-ups, see `docs/plans/2026-identity-and-pod-hardening.md`):** a partial
unique index on `profiles.id_number_hash` (airtight DB-level guarantee) is deferred until prod data is
confirmed clean of legacy live duplicates — an index migration that can fail on existing rows must not
ride along in an auto-merged fix PR; and vendor-document dedupe (persist the ID number Didit actually
verified and key the block on THAT, closing the "type a different/forged number, show the real document"
residual) needs the Didit decision-payload shape confirmed against a live run — founder/vendor-gated.
