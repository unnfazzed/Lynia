...
| KB-PROD-DEPLOY-GATE | **RECURRED — not actually resolved.** The 2026-07-17 "Recently closed" entry below was premature: the required-reviewer rule on the `production` GitHub Environment (gates both `release.yml`'s `build · migrate · deploy` and, since 2026-07-18, `deploy-admin.yml`) was re-added (or never fully removed) and has forced a fresh manual approval on effectively every production-bound commit since, flagged in the 07-17, 07-18, 07-19 (×2), and 07-20 (×2) PR-health reports. Wait times have varied wildly (minutes to 15h40m+) depending on how quickly a human notices and clicks approve. | `.github/workflows/release.yml`, `.github/workflows/deploy-admin.yml`, GitHub Environments settings | — | 6+ | Not code-fixable by a routine — no `mcp__github__*` tool can approve an Environment deployment review or edit Environment protection rules. Needs a human to either remove the required-reviewer rule on `production` for good or wire an alert (Slack/email/PagerDuty on `deployment_status` state `waiting`) so approvals happen promptly. See `docs/PR-HEALTH-REPORT-2026-07-20-0816.md` for the latest occurrence. |
