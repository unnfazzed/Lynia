...
| KB-IDENTITY-BINDING | **Identity is phone-only, so cheap SIMs → unlimited accounts** (FRAUD P2-5 multi-accounting + P2-8 phone-recycling: `verifyOtp` does `profile.upsert({ where:{ phone }})`, so a recycled MSISDN inherits the prior owner's profile+history, and a fresh SIM mints a fresh identity with no device attestation). Verified STILL LIVE 2026-07-16. **Reputation-weaponisation half now DE-FANGED by IR16-09** (customer trust tier — a sock-puppet's rating carries zero weight until it completes real orders); the remaining open surface is the *identity-cost* side (Sybil signups feeding report/issue spam + per-account-limit evasion, and the recycle-inherits-history takeover). | `apps/api/src/auth/auth.service.ts` (`verifyOtp` upsert), `packages/shared/src/phone.ts` | MEDIUM (reduced) | — (product/vendor) | **Not a conservative bug fix** — closing it fully needs device attestation (Play Integrity / App Attest) / SIM-swap detection / a re-verify-on-recycle flow, i.e. a product + vendor decision + a native build, not a code tweak on the identity-critical login path. **Recommended layered plan (docs/plans/2026-identity-and-pod-hardening.md):** L2 customer trust tier (done — IR16-09), L1 soft device-id + per-device signup throttle (code-only, next), L0 recycle-aware rebind, L3 hardware attestation (founder/vendor-gated). The trunk-0 duplicate-identity bug (a *different* normalization defect) IS fixed. |
| KB-POD-DISPUTE | **No positive proof-of-delivery dispute resolution** (FRAUD P2-6): `confirmDelivery` hard-requires the recipient's OTP, so a recipient who takes the goods then withholds the code strands the rider. A dispute path *exists* (rider `markUndelivered("refused")` is penalty-free + can raise an issue; admin sees `codeNotEntered`), but there's no way to record the trip as *delivered* over a withheld code — it stays `undelivered` though goods changed hands. Verified 2026-07-16: mitigation present, positive resolution absent. | `apps/api/src/orders/order-lifecycle.service.ts` (`confirmDelivery`/`markUndelivered`), admin issue-resolution | LOW-MEDIUM | — (product) | **Feature, not a bug fix** — a "force-complete on adjudicated dispute" admin flow is a new resolution path with its own abuse surface (an admin/rider marking undelivered trips delivered). Needs a product decision on the adjudication rule + audit, not a unilateral code change on the delivery-completion path. |
