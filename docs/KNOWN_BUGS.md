...
| KB-IDENTITY-BINDING | **Identity is phone-only, so cheap SIMs → unlimited accounts** (FRAUD P2-5 multi-accounting + P2-8 phone-recycling: `verifyOtp` does `profile.upsert({ where:{ phone }})`, so a recycled MSISDN inherits the prior owner's profile+history, and a fresh SIM mints a fresh identity with no device attestation). Verified STILL LIVE 2026-07-16. Feeds the sock-puppet half of the P1-6 reputation-farming residual and the report/issue-spam surface. | `apps/api/src/auth/auth.service.ts` (`verifyOtp` upsert), `packages/shared/src/phone.ts` | MEDIUM | — (product/vendor) | **Not a conservative bug fix** — closing it needs device attestation / SIM-swap detection / a re-verify-on-recycle flow, i.e. a product + vendor decision, not a code tweak on the identity-critical login path. The trunk-0 duplicate-identity bug (a *different* normalization defect) IS fixed; this is the inherent OTP-as-identity limitation. Left for a dedicated identity-hardening design. |
