...
| KB-PROD-DEPLOY-GATE | **RESOLVED 2026-07-17 (see Recently closed).** `release.yml`'s `build · migrate · deploy` job (`environment: production`) used to require a manual reviewer approval on every run. | `.github/workflows/release.yml`, GitHub Environments settings | — | 3 | Kept as a historical pointer; see the Recently closed row for the evidence. |
