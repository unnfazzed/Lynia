...
- [x] D-D0e **CONFIRMED MEDIUM — FIXED (LC loop D, 2026-08-02)** — `apps/merchant/app/(app)/menu/page.tsx` `onCreateStarterCategory`'s deliberate silent-swallow `catch` now writes the `ApiError` into the same `listError` state `onClearOos` (D-D0d) uses, rendered as a banner in the empty-state card the starter chips live in. Regression tests in `menu/page.test.tsx` (confirmed they fail against the pre-fix code). Ledger: LC-D05. See `docs/LC-D-REPORT-2026-08-02b.md`.
