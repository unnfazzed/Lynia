...
|---|---|---|
| **Rider** ban-evasion | **Hard-blocked (2026-07-26, IR26-01…05 — one-ID-one-account):** a national ID on another LIVE account is refused (409 `id_in_use`) at every ID-write (`completeProfile`/`auth.updateProfile`, advisory-lock serialized) and at `becomeRider` (which now requires an ID); erased tombstones/legacy dupes still route flag → manual review. **Vendor-doc dedupe shipped (IR26-04):** the document number Didit VERIFIED is hashed to `Rider.verifiedIdHash` and a `verified` outcome is held for review on mismatch-vs-typed or collision with any account's typed/verified hash — fail-open when the payload omits document data (coverage log line; confirm on first live Didit run). **DB backstop shipped (IR26-05):** partial unique index on live `id_number_hash` (migration 0039, CONCURRENTLY; ops note in the file covers the legacy-dupe failure path). | Low: a genuinely different physical document that passes the vendor — L3 attestation territory. |
