...
`apps/api/src/config/env.ts:57` (default), `:137-144` (production boot-guard)
...
`apps/admin/middleware.ts` (fail-closed proxy-auth gate now shipped — asserts an upstream
IAP/OAuth2-proxy identity header before any page/action runs), `apps/admin/app/lib/api.ts:15`
...
`infra/terraform/lb.tf:60` (backend now wires `security_policy` to `armor.tf`'s
`google_compute_security_policy.api`; still needs `terraform apply` against the live project)
...
`apps/api/src/auth/otp-sender.ts:96` (SMS, code redacted), `:109` (console, dev-only, phone masked)
...
`apps/api/src/config/env.ts:160-176`
