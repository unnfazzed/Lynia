...
| T13 | Cloud portability adapters + exit-test | ◐ partial | ◐ partial — **GCP now primary** (`CLOUD_PROVIDER=gcp`); stand-up on GCP is provisioning; the adapter seam is the portability story (the second-cloud impl was later removed) |
...
|------|------------------|------|
| **Cloud provider** — T0 | ✅ **decided (2026-06-27) — Google Cloud.** Reachable from Zimbabwe with no country-level block; nearest region **Johannesburg `africa-south1`** (lowest latency to Harare); **Google Maps** is already a dependency; Google for Startups Cloud credits available (Accelerator: Africa for the larger tier). Default is now `CLOUD_PROVIDER=gcp`; cloud-specific code stays behind the D7 adapter seam. **Unblocks** `/ship` + release, FCM push, real object storage/signed URLs, OTEL export, production OTP path — all now **provisioning execution**, not a choice. | Vendor decision — **closed** |
| **Greenlight a dev build** (not Expo Go) | ⏳ **open** → Phase 3 native map + tap-to-pin, `/qa` device pass, on-device verification of the stepper/earnings/gate. _Phase-3 code progress (2026-06-30): rider-broadcast push wired + FCM `sendEach` batching (ENG-REVIEW §5); KYC in-app browser hand-off + auto-poll-while-pending (DESIGN-REVIEW §5); mobile FCM device-token registration; **native map + tap-to-pin** for pickup/drop-off (replaces the raw lat/lng entry on the customer home, `react-native-maps`); **live tracking map** on both the customer order screen and the rider job screen (pickup + drop-off pins and the rider's GPS marker, refitting as the rider moves — the snapshot now returns redacted pickup/drop-off points) — all pending the on-device `/qa` run. Android maps need a **Google Maps API key** (`GOOGLE_MAPS_API_KEY` env / EAS secret → `app.config.ts`) — **set as an EAS secret in both `preview` and `production` environments (verified 2026-08-03)**; iOS uses Apple Maps, no key._ | Go-ahead + device |
...
- [x] **Portability check** (T13) — GCP is the primary deploy target; cloud-specific code stays behind
      the D7 adapter seam.
