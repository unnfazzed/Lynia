...
|---|---|---|
| **Cloud Scheduler jobs** (`lynia-retention-purge` daily → `POST /admin/retention/purge`, settlement autopause) | GDPR-ish retention purge **never runs**; `SCHEDULER_SERVICE_ACCOUNT` is injected but nothing calls the endpoint | **Done as of 2026-07-13** — `infra/terraform/scheduler.tf` now defines it; only a `terraform apply` remains. See `docs/GCP-PENDING-REVIEW-2026-07-13.md` §5/§8a |
| **OTel collector** Cloud Run service + `otel-collector-config` secret | No traces/metrics; SLO alert policies can't be enabled (`slo_alerts_enabled=false` or apply fails); LR9/LR11 blocked | **Done as of 2026-07-13** — `infra/terraform/otel.tf` owns the secret + IAM grant, and `release.yml` deploys the sidecar behind `OTEL_SIDECAR_ENABLED`. Only the flag flip + `terraform apply` + dashboard import remain. See `docs/GCP-PENDING-REVIEW-2026-07-13.md` §5 |
