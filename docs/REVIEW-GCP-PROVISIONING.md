...
|-----|---------|----------------|
| **P1 (partly fixed here)** | **`--allow-unauthenticated` + no `--ingress` may collide with this org's policies.** The org demonstrably runs hardened (`iam.disableServiceAccountKeyCreation`; default `*.run.app` URL disabled at the edge — see `lb.tf`). Such orgs almost always also enforce **`constraints/run.allowedIngress = internal-and-cloud-load-balancing`** (the deploy set no `--ingress`, so a new service defaulted to `all` and would be rejected) and/or **`constraints/iam.allowedPolicyMemberDomains`** (domain-restricted sharing — blocks the `allUsers` binding `--allow-unauthenticated` adds → deploy step fails). Either reds the **first** deploy *after* a green build/migrate — the exact "green CI, dead service" class the original review targeted. | ✅ **`--ingress internal-and-cloud-load-balancing` added** to `release.yml` (strictly correct — the ALB fronts the service). **Still verify** the `allUsers` path before arming: `gcloud resource-manager org-policies list --project lynia-500911`. If domain-restricted sharing is enforced, drop `--allow-unauthenticated` and grant invoker to an org-allowed principal (or an org-policy exception/tag). Left in place since the ALB→serverless-NEG path needs `allUsers` invoker when the policy is *not* enforced. |
