...
| `@react-native-firebase/app` + `/crashlytics` | Native (moderate) + JS | Firebase Android SDK (BoM 33.12.0) + `firebase-crashlytics-ndk` → `lib/`/`dex`; ~139 KiB to the Hermes bundle | **A deliberate exception to the one-SDK-per-use-case rule** — it is a *second* crash reporter alongside Sentry. Justified by exactly one gap: Sentry arms from JS and therefore cannot see a crash that happens before its own `init()` (MOB-BOOT-04, `docs/COLD-START-CRASH-RCA-2026-08-21.md`), whereas Crashlytics arms from a native ContentProvider pre-JS. If boot-crash risk stops being live, this is the one to drop — not Sentry. See `docs/CRASHLYTICS.md`. |
