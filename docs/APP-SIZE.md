...
| **OTA** | JS + asset bundle | `mobile-ota.yml` — **armed but not functional** (`REL-01`/`REL-02`, `docs/KNOWN_BUGS.md`); zero updates published to date | Only lands on binaries whose native fingerprint matches (`runtimeVersion: fingerprint`). Native/SDK changes need a store release, not an OTA. Until `REL-01` is decided, treat OTA size work as unshippable — every byte still ships via the store lane. |
