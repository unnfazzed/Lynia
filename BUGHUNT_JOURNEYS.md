...
2. ✅ OTP unhappy paths: expired code, wrong code (attempt cap), too many sends (rate limits), re-request, lost response/retry. — F-15
...
5. ✅ Compose an order (waypoints, items, note, photo, declared value, proposed fare) within service corridor. — F-05
6. ✅ Broadcast the order (opens `open_for_offers`), incl. idempotent double-tap / timeout-retry.
7. ✅ Order rejected: outside service corridor; account on hold; validation failure. — F-01
...
12. ✅ Track the assigned ride: live rider position (WS + REST snapshot), status milestones, phone reveal window. — F-02, F-16
13. ✅ Relay the delivery code to the recipient; re-issue (rotate) the code after a lockout. — F-03
...
16. ✅ Delivery completes → rate the rider (closes order) or auto-close after rating window. — F-09
...
20. ✅ Cold start / app-kill mid-flow: restore active order (`mine/active-order`), replay push deep link. — F-04
...
22. ✅ Trip history + in-app notifications feed (derived, read-only). — F-09
...
27. ✅ KYC resubmit (retry) within attempt cap (A-02 lock after 2 declines) + support dead-end copy. — F-13
...
45. ✅ KYC review + decision (approve / decline w/ reason / expire / reset); attempt-lock; audit-in-tx. — F-14
...
50. ✅ Disputes queue: list / detail / resolve (refund / rider_strike / close); CAS on double-resolve; refund cap. — F-11
...
53. ✅ Offer-window expiry (BullMQ job idempotent by jobId) + DB reconciler backstop. — F-12, F-17
54. ✅ Rating auto-close (delayed job) + DB reconciler backstop. — F-12, F-17
...
58. ✅ "Notify me" waiting-list drain when a rider comes online nearby. — F-18
...
61. ✅ Client-RUM metrics ingest (strict schema, clamped labels). — F-19

## E. New since Pass 1 (PR #190 delta)

62. ✅ OTP verify lost-response retry: 60s hash-only grace window (re-verify same code → fresh session); replay/brute-force bounds; Redis vs in-memory store parity. — F-15
...
_Status: Pass 2 complete — all 66 journeys audited. New findings F-11…F-19 in `BUGHUNT_FINDINGS.md`; Pass-1 F-01…F-10 all re-verified still-open._
