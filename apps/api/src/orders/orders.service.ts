...
export class OrdersService implements OnModuleDestroy {
  ...
  async historyForUser(userId: string) {
    const orders = await this.prisma.order.findMany({
      // Only successfully-completed orders belong in the Orders/Trips history lists — drafts and
      // in-flight orders, plus cancelled/expired/undelivered, are excluded (COMPLETED_ORDER_STATUSES).
      // Filtering in SQL (not client-side after the fact) keeps the `take: 50` cap meaningful: 50
      // completed rows, not 50 recent-of-any-status of which only a handful are completed.
