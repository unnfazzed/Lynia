...
export default function HomeScreen(): React.ReactElement {
  ...
  const nums = [pickupLat, pickupLng, dropLat, dropLng].map(Number);
  const haveBothPoints = pickupLat !== "" && dropLat !== "" && nums.every(Number.isFinite);
  const quote = haveBothPoints
    ? quoteFare({ lat: Number(pickupLat), lng: Number(pickupLng) }, { lat: Number(dropLat), lng: Number(dropLng) })
    : null;
  ...
  const submit = async (): Promise<void> => {
    setError(null);
    const candidate = {
      pickup: { point: { lat: Number(pickupLat), lng: Number(pickupLng) }, landmark: pickupLandmark.trim(), contactPhone: pickupPhone.trim() },
      dropoff: { point: { lat: Number(dropLat), lng: Number(dropLng) }, landmark: dropLandmark.trim(), contactPhone: dropPhone.trim() },
      itemDescription: itemDescription.trim(),
      declaredValue: Number(declaredValue || "0"),
      proposedFare: Number(proposedFare),
    };
  ...
  return (
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Button label="Broadcast request" onPress={submit} loading={busy} />
